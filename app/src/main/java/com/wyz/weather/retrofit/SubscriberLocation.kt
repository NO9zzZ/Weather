 ...
 */
abstract class SubscriberLocation(view: BaseView?) :
    DisposableSubscriber<LocationBean?>() {
    ...
    override fun onNext(t: LocationBean?) {
        if (SUCCESS_CODE == t?.getStatus()) {
            onHandleSuccess(t, t?.getInfo())
        } else {
    ...
    abstract fun onHandleSuccess(result: LocationBean?, message: String?)
